...
def rasterize(parsed):
  ...
  """
  # We can assume that there are two objects of interest to predict  
  ...
  interaction_tracks = decoded_example['state/objects_of_interest'] # (128,)
  interact_tracks_before_mask = tf.where(interaction_tracks == 1).numpy() # indices before masking
  interaction_tracks = tf.boolean_mask(interaction_tracks, is_valid)
  interact_tracks_after_mask = tf.where(interaction_tracks == 1).numpy() # indices after masking
  interact_tracks_before_mask = np.squeeze(interact_tracks_before_mask, axis = 1)
  interact_tracks_after_mask = np.squeeze(interact_tracks_after_mask, axis = 1)
  ...
  batch_images = np.zeros((len(interact_tracks_before_mask), 224,448, 3), dtype=np.float32)
  if len(interact_tracks_before_mask) != 2 or len(interact_tracks_after_mask) != 2:
    raise Exception('WARNING! TRACKS TO PREDICT ARE NOT LENGTH 2!')
  for track_index, track in enumerate(interact_tracks_before_mask):
    if not current_is_valid[track_index]:
      print("WARNING! Found a track that is not valid in current frame!")
      batch_images[track_index] = None
      continue
    track_after_mask = interact_tracks_after_mask[track_index]
    ...
    other_track = interact_tracks_after_mask[1-track_index]
    ...
    for i in range(11):
      ...
      cv2.fillPoly(out_img, pts[i][track_after_mask:track_after_mask+1], color = (0,1,0), lineType=cv2.LINE_AA, shift=CV2_SHIFT)
